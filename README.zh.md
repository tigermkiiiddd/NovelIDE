...
- **交互式澄清**：当 AI 遇到真正的歧义——用户输入前后矛盾、缺少关键参数、或执行方向分歧时——它会通过问卷面板提出针对性的单选/多选问题，而不是瞎猜。
...
### 软工作流（Soft Workflow）
介于裸提示词与硬工具调用之间的控制层。作者定义结构化执行规程——步骤、检查点、反模式——在不增加工具复杂度的情况下引导 AI 完成复杂创作任务。

- **步骤化执行**：将写作任务拆解为可执行阶段（如上下文召回 → 大纲对齐 → 正文生成 → 质量检查）。
- **内置检查点**：在每个阶段验证故事目标、角色状态和伏笔回收情况。
- **反模式防护**：预防"展示而非讲述"失效、对话无目的、行为无动机等常见问题。
- **与硬工具混合**：软工作流负责创意流程编排，仅在真正需要数据读写时才解锁硬工具。
...
### 全局与项目灵魂（Global & Project Soul）
在两个独立层级配置 AI 的创作人格与风格：

- **全局灵魂（Global Soul）**：应用级、跨项目的偏好设置。承载长期的协作习惯、沟通风格、可复用的审美倾向，以及高重要度的纠正，跟随你穿越所有项目。
- **项目灵魂覆盖（Project Soul Override）**：项目级人格文件（`soul.md`），记录当前项目的特殊要求——语气、视角、 prose 密度、题材约束。存在冲突时自动覆盖全局灵魂。
- **清晰隔离**：故事事实、角色声音、专有名词、世界规则、情节状态留在项目资产内；默认只有风格和协作偏好可以跨项目继承。
...
### 7. 软工作流
在裸提示词与硬工具调用之间，软工作流让作者定义 AI *如何*执行——而不仅仅是*写*什么。正文写作流程、正文扩写等 Skill 都是软工作流的实例：它们提供明确的触发条件、执行原则、检查清单和反模式，但不需要代码级别的工具调用。这让作者能控制 AI 的创作规程本身。
