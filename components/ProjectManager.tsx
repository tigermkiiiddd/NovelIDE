...
首先，从【简介/核心梗】和【用户额外指令】中提取所有明确的约束：

1. **主角结构约束**：
   - 关键词识别：单主角/双主角/多主角/群像/无主角
   - 示例："群像" → 约束=多主角并重，禁止单一主角视角

2. **感情线约束**：
   - 关键词识别：无CP/单女主/单男主/多女主/后宫/百合/耽美
   - 示例："无CP" → 约束=无感情线，禁止推荐任何恋爱相关标签

3. **风格约束**：
   - 关键词识别：黑暗/治愈/爽文/虐文/轻松/严肃/幽默
   - 示例："黑暗压抑" → 约束=黑暗风格，禁止推荐"轻松幽默"

4. **玩法约束**：
   - 关键词识别：无系统/纯技巧/无金手指/硬核
   - 示例："无系统" → 约束=不推荐"系统流"、"签到流"等标签
...
- ✅ 推荐与约束一致的标签
- ❌ 绝对不推荐与约束冲突的标签
- 如果用户已选标签与约束冲突，保留用户已选标签（用户选择优先）

**Step 3: 简介扩写（保留核心）**

- 如果简介已有内容：提取核心关键词（主角类型、世界观、冲突），在此基础上扩写
- 如果简介为空：根据题材和约束生成
...
3. **完善简介**：
   - 如果简介已有内容：在原基础上扩写，**必须保留**用户提到的关键设定（如"群像"、"无CP"、"黑暗"等）
   - 如果简介为空：根据题材和标签生成 100-200 字简介
