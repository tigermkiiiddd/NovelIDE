          ...
          items: {
            ...
            properties: {
              ...
              tags: {
                type: 'array',
                items: { type: 'string' },
                description: '伏笔标签（至少1个）。常见类型：["身世"]、["物品/宝物"]、["能力/力量"]、["关系/恩怨"]、["事件/阴谋"]、["预言/梦境"]、["感情线"]、["反转铺垫"]等。也可以标注爽点等级：["爽点:小"]、["爽点:中"]、["爽点:大"]，或组合使用：["爽点:中", "突破"]'
              },
              notes: {
                type: 'string',
...
#### 爽点追踪（通过伏笔系统）
爽点也可以通过伏笔系统追踪。在识别到爽点场景时：
- 在伏笔的 \`tags\` 字段中添加爽点等级：\`爽点:小\`、\`爽点:中\`、\`爽点:大\`
- 用 \`planted\` 标记爽点规划（本章出现的小高潮），用 \`resolved\` 标记已实现的爽点
- 示例：打脸场景 → \`tags: ["爽点:小", "打脸"]\`，突破场景 → \`tags: ["爽点:中", "突破"]\`
- 也可以给已有伏笔添加爽点标签，表示伏笔回收时的爽点等级
