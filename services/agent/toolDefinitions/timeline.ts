...
调用此工具前，必须先用读取工具查看现有结构和时间线位置：
- outline_getVolumes() - 查看现有卷
- outline_getChapters() - 查看现有章节
- outline_getEvents() - 查看最新事件，确认时间线当前位置（最后事件的 day/hour）
- outline_getEvents(chapterIndex) - 查看目标章节内的事件

**⚠️ 时间戳规则**：新事件的时间戳必须接续时间线当前位置（最后事件之后）。禁止自己编造时间！必须基于 outline_getEvents 返回的最后事件时间来计算。
...
【数量】共12个事件
【时间线位置】最后事件在第3天22时，新事件从第4天开始"

**⚠️ instructions 中的【时间线位置】必须基于 outline_getEvents 的查询结果填写，不要自己猜测！**`
        }
      },
