...
## ⚠️ 严禁创建档案的角色类型（违反会导致系统混乱）
以下角色类型**绝对不应该**创建档案，如果遇到请直接跳过：
- **泛指群体**：如"流氓们"、"路人"、"年轻女孩"、"村民们"、"士兵们"等
- **描述性称呼**：如"神秘人"、"老者"、"年轻女子"、"黑衣人"等（除非是正式代号）
- **一次性龙套**：只在一个场景出现、没有后续剧情意义的角色
- **未命名角色**：没有正式名称的角色

## ✅ 应该创建档案的角色
只有在以下条件下才应该调用此工具：
1. 角色在 02_角色档案 目录下有正式的角色设定文档
2. 角色有具体的**个人名称**（如"苏清月"、"林逸"）
3. 角色是**主要角色、重要配角或常驻角色**
4. 角色会在多个章节中反复出现
...
## 覆盖型分类（状态、属性、目标、技能）
- **只提取最终状态**，不要包含演变过程
- value 必须是单一的确定值，禁止使用 "A -> B -> C" 这样的演变格式
- ❌ 错误示例："商业街VIP洗手间 -> 商业街人群 -> 公寓"
- ✅ 正确示例："公寓"
- ❌ 错误示例："白色连衣裙（破损） -> 清洁工制服 -> 休闲装"
- ✅ 正确示例："休闲装"
- action 设为 'update'
