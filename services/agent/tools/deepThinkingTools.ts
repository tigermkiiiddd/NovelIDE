...
export const deepThinkingTool: ToolDefinition = {
  ...
  function: {
    name: 'deep_thinking',
    description: `深度分析工具。创意性设计工作的默认工具，不是"只有复杂才用"。
...
必须使用的场景（满足任一即触发）：
- 设计角色、场景、冲突、情节线、世界观规则
- 多个可行方向需要选择，或约束之间有张力
- 用户对已有内容表达不满、纠正、或推翻之前的设定
- 规划章节结构、角色弧光、节奏分布
- 用户说"仔细想想"/"深度分析"/"认真考虑"/"想清楚"/"推倒"

可以不用的场景：
- 明确的指令执行（"改成X"、"加个角色叫Y"）
- 简单信息查询、纯文本润色

误判代价：该用没用 → 方案浅薄用户不满（后果严重）；不该用但用了 → 多花几秒（后果轻微）。宁可多调也不要跳过。`,
    parameters: {
      type: 'object',
      properties: {
        action: {
