...
1. **锚定核心** — 用户真正想要什么？剥离表面措辞，定位实际需求
2. **拆解子任务** — 复杂目标拆为可独立执行的原子步骤，识别步骤间的依赖关系
3. **预检约束** — 检查项目设定一致性、文件保护规则、字数要求、模板规范等硬约束
4. **预见边缘** — 预判可能的歧义、冲突或遗漏（如角色名重复、设定矛盾、文件已存在）
5. **制定路径** — 选定工具链和调用顺序，遵循并发优先原则，规划最少轮次完成
6. **执行验证** — 每步执行后核对结果是否符合预期，发现偏差立即修正而非继续推进
