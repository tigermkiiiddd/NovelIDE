...
- 全局 Soul 直接编辑项目中的 98_技能配置/skills/核心/global-soul.md 文件。
- 项目 Soul 覆盖直接编辑项目中的 98_技能配置/skills/核心/soul.md 文件。
...
- **全局 Soul**：直接编辑 98_技能配置/skills/核心/global-soul.md
...
export const constructSystemPrompt = (
  ...
  // 1.1 Resolve Soul (从文件系统读取显式的 global-soul.md 和 soul.md)
  const findSoulFile = (name: string) => {
    let f = skillsFolder
      ? files.find(file => {
          if (file.name !== name || file.type !== FileType.FILE) return false;
          const parentFolder = files.find(p => p.id === file.parentId);
          return parentFolder?.name === '核心';
        })
      : null;
    if (!f) f = files.find(file => file.name === name && file.type === FileType.FILE);
    return f;
  };

  const globalSoulFile = findSoulFile('global-soul.md');
  const projectSoulFile = findSoulFile('soul.md');

  const globalSoulContent = globalSoulFile?.content?.trim() || DEFAULT_SOUL;
  const projectSoulContent = projectSoulFile?.content?.trim() || '';

  const isEmptyProjectSoul =
    !projectSoulContent ||
    projectSoulContent === DEFAULT_SOUL.trim() ||
    projectSoulContent === PROJECT_SOUL_TEMPLATE.trim();

  const projectSoulOverride = projectSoulContent && !isEmptyProjectSoul
    ? `## 项目 Soul 覆盖\n\n${projectSoulContent}`
    : '';

  const soulInstruction = [globalSoulContent, projectSoulOverride].filter(Boolean).join('\n\n---\n\n');
