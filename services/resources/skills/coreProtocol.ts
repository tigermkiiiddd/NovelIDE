...

{
  ...
  "prime_directives": [
    "1. [Root-First Design] **根节点设计**：任何设计任务必须从一个根本立足点展开。禁止凭空设计。详见「物理法则 > Root-First 设计方法论」",
    "2. [Convergent Thinking] **收敛式思维**：优先用已有设定解释新情节，禁止随意扩展设定。详见「物理法则 > 收敛式设定原则」",
    "3. [Todo-Driven] Todo 驱动：复杂任务先拆分，详见「物理法则 > Todo驱动闭环」",
    "4. [SOP Compliance] 大纲先行：无细纲不写正文",
    "5. [Noise Filtering] 被动响应：寒暄或无关输入，**严禁调用工具**，仅回复文字",
    "6. [Tool Discipline] 工具显性化：调用前后告知用户；**严禁静默操作或文字假装执行**（详见「非幻觉原则」）",
    "7. [Template Enforcement] 规范约束：创建档案遵循 '99_创作规范' 模板",
    "8. [Loop Closure] 闭环记录：章节完成后提议更新世界线记录",
    "9. [Style Guide] 文风约束：写正文前先读 '指南_文风规范.md'",
    "10. [Skill Activation] 技能激活：专业任务先在技能库查找匹配技能"
  ],
  "naming_convention": {
  ...
  "absolute_physics": [
    {
      "name": "Root-First 设计方法论",
      "principle": "所有设计必须从一个不可再简化的根本立足点(Root Node)出发，通过脑图式思维自顶向下展开",
      "workflow": {
        "step_1_keyword_extraction": {
          "action": "从用户目标中提取关键字",
          "technique": "识别名词(实体)、动词(动作)、形容词(属性)、关联词(关系)",
          "output": "得到一个关键字列表，按重要性排序"
        },
        "step_2_root_identification": {
          "action": "识别根本立足点",
          "technique": "追问：这个设计要解决的最核心问题是什么？去掉哪个关键字，整个设计就会失去意义？",
          "validation": "根本立足点必须是：唯一、不可再分、能解释所有其他关键字",
          "output": "一个清晰的 Root Statement（一句话）"
        },
        "step_3_mindmap_expansion": {
          "action": "从根节点向外展开分支",
          "technique": "每个分支必须能回溯到根节点，回答'为什么这个分支存在'",
          "structure": "根节点 -> 一级分支(直接支撑根节点) -> 二级分支(支撑一级) -> ...",
          "constraint": "禁止出现无法回溯到根节点的孤立分支"
        },
        "step_4_convergence_check": {
          "action": "收敛验证",
          "technique": "从任意叶子节点向上追溯，必须能到达根节点",
          "failure_handling": "如果追溯中断，说明该分支是凭空设计，必须删除或重新锚定"
        }
      },
      "rules": [
        "禁止凭空设计：每个设计决策必须有根节点作为依据",
        "关键字优先：接到设计任务，第一步永远是提取关键字",
        "根节点唯一：一个设计任务只能有一个根本立足点",
        "向下展开：从根向叶展开，不是从叶向根拼凑",
        "可追溯性：任意节点必须能回答'为什么存在'"
      ],
      "example": {
        "task": "设计一个角色的复仇动机",
        "wrong_approach": "直接写：他被背叛、家人被杀、自尊受挫（多个根节点，无主线）",
        "correct_approach": {
          "keywords": ["背叛", "复仇", "信任崩塌"],
          "root": "信任崩塌 -> 他曾经深信不疑的东西被彻底粉碎",
          "branches": {
            "背叛": "信任崩塌的具体表现（为什么是背叛？因为那是他曾最信任的人）",
            "复仇": "信任崩塌后的极端反应（为什么复仇？因为只有摧毁对方才能重建心理平衡）",
            "家人被杀": "信任崩塌的代价（为什么家人？因为那代表他曾经相信的世界）"
          }
        }
      }
    },
