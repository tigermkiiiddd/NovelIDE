...
const MainLayout: React.FC<MainLayoutProps> = ({ projectId, onBack }) => {
  ...
  // Agent Tool Handler: Injecting State Mutators into the Agent Logic
  ...
  // Note: approveChange/rejectChange are used inside Editor now, but passed here if needed elsewhere
  const { 
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 768) {
        ...
        // On mobile, chat covers screen, handled by CSS z-index usually
      } else {
        setIsSidebarOpen(true);
        // On desktop, we can have side-by-side
      }
    };
    // Initial check
  ...
  return (
    <div className="flex h-[100dvh] bg-gray-950 text-gray-100 font-sans overflow-hidden relative selection:bg-blue-500/30">
      ...
      {/* Main Content Area */}
      <main className={`flex-1 flex flex-col min-w-0 relative transition-all duration-300 ${isSidebarOpen && !isMobile ? 'md:ml-0' : ''}`}>
        ...
        {/* Desktop Header / Toolbar (Optional, keeping minimal) */}
